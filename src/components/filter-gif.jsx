...
function FilterGif({ alignLeft = false, showTrending = false }) {
  const { filterGifs, setFilterGifs } = GifState();
  return (
    <div
    ...
    >
      {showTrending && (
        <span className="flex gap-2 my-3">
          ...
          <span className="text-gray-400 font-semibold">Trending</span>
        </span>
      )}
      ...
      <div className="flex min-w-80 rounded-full bg-gray-800">
        {filters.map((f) => {
          return (
            <span
              onClick={() => setFilterGifs(f.value)}
              className={`${
                filterGifs === f.value ? f.background : ""
              }font-semibold py-3 w-1/3 text-center rounded-full cursor-pointer`}
              key={f.title}
            >
              {f.title}
            </span>
